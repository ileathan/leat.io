...
*
*  Find leatClient is associated server data and load them.
*  along with a little hack to make sure we load content, lC.load().
*/
...
;
pRef = fromStorage('ref') || toStorage({ref: pRef})
;
/*
* Redirect legacy domains to leat.io ones.
*/
...
;
/*
* Remove numbers so its not so ugly. (can preserve history 'state' here later)
*/
...
*
* -note: swap to just #pie so we can remove #pie display:none hack.
*/
lC.refreshStats = () => {
...
*/
lC.appendTran = d => {
  const flag = ["REF PAYMENT", "ref-payment", "MINED FOR", "minedfor-payment", "TRANSFER", "transfer"][d.type]
  ;
  var date = new Date().toLocaleString()
  , colotT = d.amount < 0 ? "red" : (d.amount = 1) && "green" 
  , colorU = lC.toColor(d.user) 
  , html = '<li>' + date + ' '
             + '<span style="color:"' + colorT  + '">'
               + '<b>'+ d.amount + (d.received ? ' received from ' : ' sent to ') + '</b>' 
             + '</span>'
             + '<font style="color:' + colorU + '">' + d.user + '</font>'
             + '<span class="payment "' + flag[d.type+1] + '>'+ flag[d.type] + '</span>'
         + '</li>'
  ;
...
*
* Type just represents the mode they want, 3 options total.
*
*/
...
*/
lC.transfer = () => {
  let amount = (a = $('#transfer-amount-input')).val(), username = (t = $('#transfer-to-input')).val();
  lC.socket.emit("transfer", {amount, username}, (res, err) => {
    if(res) {
      $('#transfer-info').text("Sent " + username + " " + amount).css('color', 'gold');
      // Record the transfer to the transaction log.
      lC.appendTran({user: username, amont: -amount, type: 4});
      a.val(''); t.val('');
